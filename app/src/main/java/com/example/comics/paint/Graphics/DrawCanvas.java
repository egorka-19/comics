package com.example.comics.paint.Graphics;
...
import com.example.comics.paint.Tools.SelectionTool;
import com.example.comics.paint.Tools.Tool;
public final class DrawCanvas extends View {
    private String cloudText;
    private float cloudX = 300, cloudY = 300; // Default cloud position
    ...
    private void drawCloud(Canvas canvas) {
        ...
        cloudPaint.setColor(Color.BLACK);
        cloudPaint.setStyle(Paint.Style.FILL);

        Path cloudPath = new Path();
        cloudPath.addCircle(cloudX, cloudY, 150, Path.Direction.CW);
        cloudPath.addCircle(cloudX + 150, cloudY, 150, Path.Direction.CW);
        cloudPath.addCircle(cloudX + 75, cloudY - 100, 130, Path.Direction.CW);

        canvas.drawPath(cloudPath, cloudPaint);

        cloudPaint.setColor(Color.WHITE);
        cloudPaint.setTextSize(50);
        canvas.drawText(cloudText, cloudX + 30, cloudY, cloudPaint);
    }
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        if (!drawMinimal) {
            ...
            paint.setStrokeWidth(5 / panTool.scaleFactor);
        ...
        // 🗨️ Draw the cloud shape if available
        if (cloudText != null) {
            drawCloud(canvas);
        }
